...
function everSentKey(tier: "14d" | "7d" | "3d", id: string): string {
  return `bricolo_sent_notif_ever_reminder_${tier}_${id}`;
}
...
describe("checkSavedEventNotifications", () => {
  ...
  it("catches up on a tier crossed while the app was closed for months, without also bursting every less-urgent tier as a separate notification", () => {
    // App wasn't opened again until 2 days before the project deadline:
    // both the 14-day and 7-day thresholds were crossed unseen while closed.
  ...
  it("does nothing for a project date that has already passed", () => {
