...
export const ResultsDeckApple: React.FC<ResultsDeckAppleProps> = React.memo(({
  ...
  const wsText = language === "it"
    ? `Ho trovato il prodotto giusto per il mio progetto: ${currentGift?.title} (${currentGift?.price})! Guarda qui su Amazon: ${buildAmazonUrl(currentGift?.amazonSearchQuery || "", country, currentGift)}`
    : `I found the right product for my project: ${currentGift?.title} (${currentGift?.price})! Check it out: ${buildAmazonUrl(currentGift?.amazonSearchQuery || "", country, currentGift)}`;
  ...
  return (
    <div className="relative w-full h-full bg-[#F2F2F7] flex flex-col justify-between pt-[max(6px,env(safe-area-inset-top,0px))] pb-[max(8px,env(safe-area-inset-bottom,0px))] px-[max(12px,env(safe-area-inset-left,0px))] pr-[max(12px,env(safe-area-inset-right,0px))] select-none max-w-lg sm:max-w-xl md:max-w-2xl mx-auto overflow-hidden font-sans gpu-layer">
      ...
      <div className="flex items-center justify-between shrink-0 pt-0.5 pb-2 border-b border-[#E5E5EA]">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <span className="text-[9px] sm:text-[10px] font-extrabold text-[#E8590C] uppercase tracking-wider flex items-center gap-1">
              ...
              BRICOLO AI • 3 SELEZIONI PERFETTE
            </span>
        ...
        <button
          onClick={onStartOver}
          className="py-1 px-2.5 sm:py-1.5 sm:px-3 rounded-full bg-white border border-[#E5E5EA] text-[#E8590C] hover:bg-[#E5E5EA] active:scale-95 transition-transform text-[11px] sm:text-xs font-bold shadow-2xs cursor-pointer shrink-0 ml-1 gpu-layer"
        >
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2 w-full shrink-0 pt-0.5">
                  <a
                    ...
                    className="py-2.5 sm:py-3 px-2 rounded-[16px] bg-white border-2 border-[#E8590C] hover:bg-[#F2F2F7] active:scale-[0.98] text-[#E8590C] font-black text-[11px] sm:text-xs flex items-center justify-center gap-1.5 cursor-pointer shadow-2xs transition-all uppercase tracking-wider gpu-layer"
                  >
                  ...
                  <a
                    ...
                    style={{
                      backgroundColor: isPrimary ? "var(--brand-coral)" : "#E8590C",
                      borderColor: isPrimary ? "var(--brand-coral)" : "#E8590C",
                    }}
                  >
      ...
      <div className="shrink-0 pt-1.5 border-t border-[#E5E5EA] space-y-1">
        <div className="grid grid-cols-2 gap-2">
          ...
          <button
            ...
            className="py-2.5 sm:py-3 px-2 rounded-2xl bg-white border border-[#E5E5EA] hover:border-[#E8590C] text-[#000000] font-bold text-xs flex items-center justify-center gap-1.5 transition-colors cursor-pointer shadow-2xs active:scale-[0.98] gpu-layer"
          >
            <MessageCircle className="w-4 h-4 text-[#E8590C] shrink-0" />
            <span className="truncate">
              {copiedWs
                ? (language === "it" ? "Copiato & Aperto!" : "Copied & Opened!")
                : (language === "it" ? "Copia Link" : "Copy Link")}
            </span>
            {copiedWs && <Check className="w-4 h-4 text-[#E8590C] shrink-0" />}
          </button>
        ...
        {/* Promemoria opzionale — appare SOLO qui, dopo che l'utente ha
            ...
            data), nome progetto facoltativo. Questo e cio che trasforma
              ...
              <div className="p-3 rounded-2xl bg-white border-2 flex flex-col gap-2" style={{ borderColor: "var(--brand-coral)" }}>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    placeholder={language === "it" ? "Nome progetto (facoltativo)" : "Project name (optional)"}
