...
const COPY = {
  it: {
    ...
    placeholder: "Es. Devo appendere una mensola pesante su un muro in cartongesso",
    submit: "TROVA L'ATTREZZO",
    clear: "Cancella",
  ...
  en: {
    ...
    label: "What you said",
    placeholder: "E.g. I need to hang a heavy shelf on a plasterboard wall",
    submit: "FIND THE TOOL",
    clear: "Clear",
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed bottom-0 left-0 right-0 z-[9991] mx-auto flex max-w-lg select-none flex-col items-center gap-5 rounded-t-[32px] border-t border-[#332A1E] bg-[#1A1611]/95 p-6 pb-[max(24px,env(safe-area-inset-bottom,0px))] font-sans shadow-[0_-12px_40px_rgba(0,0,0,0.4)] backdrop-blur-lg gpu-layer"
          >
            <div className="-mt-1 h-1.5 w-12 shrink-0 rounded-full bg-[#332A1E]" />

            <div className="flex w-full items-center justify-between">
              <div className="flex items-center gap-2 text-[#FF8A1F]">
                <Sparkles className="h-4 w-4 fill-[#FF8A1F]" />
            ...
            <div className="flex flex-col items-center gap-4 py-2">
              ...
              >
                {isListening && (
                  <motion.span
                    ...
                    className="absolute inset-0 rounded-full border-2 border-[#FF8A1F]"
                  />
                )}
                ...
                <Mic className={`h-9 w-9 stroke-[2.5] ${isListening ? "text-white" : "text-[#FF8A1F]"}`} />
              </button>
              ...
              <div className="flex h-8 items-center justify-center gap-1.5">
                {Array.from({ length: BAR_COUNT }).map((_, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className={`h-[30px] w-1.5 rounded-full gpu-layer ${isListening ? "bg-[#FF8A1F]" : "bg-[#332A1E]"}`}
                      style={{
              ...
              <span className="text-xs font-semibold text-[#A89880]">
                {isListening ? t.listening : t.idle}
              </span>
            </div>

            {errorMsg && (
              <p className="px-2 text-center text-[11px] font-medium text-[#FFC08A]">{errorMsg}</p>
            )}

            <div className="w-full space-y-2">
              <label className="block text-left text-[11px] font-bold uppercase tracking-wider text-[#A89880]">
                {t.label}
              </label>
              ...
              <div className="relative w-full">
                <textarea
                  ...
                  className="w-full resize-none rounded-[18px] border border-[#332A1E] bg-[#221C14] p-3.5 text-base font-bold leading-snug text-[#F5F1EA] placeholder-[#A89880] transition-colors focus:border-[#FF8A1F] focus:outline-none sm:text-lg"
                />
                {displayText && (
                  <button
                    type="button"
                    ...
                    className="absolute right-3 top-3 cursor-pointer rounded-full bg-[#332A1E] p-1 text-[#A89880] transition-colors hover:text-[#F5F1EA]"
                    title={t.clear}
                    aria-label={t.clear}
                  >
