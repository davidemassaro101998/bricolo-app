    ...
    try {
      ...
      recognition.onerror = (event: any) => {
        ...
        if (event.error !== "no-speech" && event.error !== "aborted") {
          setErrorMsg(t.micPermissionDenied);
        }
      };
        ...
        <>
          ...
          >
            ...
            <div className="w-full flex items-center justify-between">
              <div className="flex items-center gap-2 text-[#E8590C]">
                <Sparkles className="w-4 h-4 fill-[#E8590C]" />
                <span className="text-xs font-bold uppercase tracking-wider">
                  {t.aiVoiceAssistantTitle}
                </span>
              </div>
              <button
                type="button"
            ...
            <div className="flex flex-col items-center gap-4 py-2">
              <button
                ...
                className={`relative w-20 h-20 rounded-full flex items-center justify-center transition-all cursor-pointer shadow-lg active:scale-95 ${
                  isListening
                    ? "bg-[#E8590C] text-white shadow-[0_0_28px_rgba(232,89,12,0.5)]"
                    : "bg-[#F2F2F7] text-[#E8590C] border border-[#E5E5EA]"
                }`}
              >
                ...
                {isListening && (
                  <motion.span
                    ...
                    className="absolute inset-0 rounded-full border-2 border-[#E8590C]"
                  />
                )}
                {isListening ? (
              ...
              {/* Dynamic Waveform Bars in Bricolo Orange #E8590C.
                ...
                {[0.4, 0.9, 0.6, 1.0, 0.7, 0.8, 0.5].map((heightScale, idx) => (
                  <motion.div
                    ...
                    className={`w-1.5 h-[30px] rounded-full gpu-layer ${
                      isListening ? "bg-[#E8590C]" : "bg-[#E5E5EA]"
                    }`}
                  />
                ))}
              </div>
              ...
              <span className="text-xs font-semibold text-[#68686D]">
                {isListening ? t.listeningHint : t.tapMicHint}
              </span>
            </div>
            ...
            <div className="w-full space-y-2">
              ...
              <div className="relative w-full">
                <textarea
                  ...
                  placeholder={t.voiceIdeaPlaceholder}
                  className="w-full p-3.5 rounded-[18px] bg-[#F2F2F7] text-[#000000] placeholder-[#68686D] text-base sm:text-lg font-bold leading-snug border border-[#E5E5EA] focus:outline-none focus:border-[#E8590C] transition-colors resize-none"
                />
                {transcript && (
                  <button
                    type="button"
            ...
            <button
              ...
              className="w-full py-4 px-6 rounded-[20px] bg-[#E8590C] hover:bg-[#C24A08] text-white font-black text-sm uppercase tracking-wider flex items-center justify-center gap-2.5 shadow-[0_8px_20px_rgba(232,89,12,0.3)] raised-surface active:scale-[0.98] transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed border border-[#E8590C] shrink-0 mt-1"
            >
