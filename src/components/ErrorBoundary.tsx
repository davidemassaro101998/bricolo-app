...
> {
  ...
  componentDidCatch(error: Error, info: React.ErrorInfo) {
    reportError(error, { componentStack: info.componentStack, app: "Bricolo AI" });
  }
  ...
  handleReset = () => {
    ...
    try {
      ...
      localStorage.removeItem("bricolo_home_form_state");
    } catch (e) {
      // ignore
    }
    ...
    if (this.state.hasError) {
      ...
      return (
        <div className="fixed inset-0 z-[999] bg-[#1C1520] flex flex-col items-center justify-center gap-4 px-8 text-center font-sans">
          ...
          <h2
            className="text-xl text-[#F5F1EA]"
            style={{ fontFamily: "var(--font-display)", fontWeight: 600 }}
          >
            {isIt ? "Qualcosa è andato storto" : "Something went wrong"}
          </h2>
          <p className="text-sm text-[#97908A] max-w-xs">
            {isIt
