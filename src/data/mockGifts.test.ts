...
describe("generateSmartFallbackGifts", () => {
  ...
  it("generates non-Italian copy when a non-Italian language is requested", () => {
    ...
    for (const gift of gifts) {
      expect(gift.title.toLowerCase()).not.toContain("officina");
    }
  });
  ...
  it("matches the garden topic override regardless of language", () => {
    for (const language of languages) {
      const gifts = generateSmartFallbackGifts(quiz({ extraDetails: "progetto giardino" }), IT_COUNTRY, language);
      expect(gifts.some((g) => g.category === "garden")).toBe(true);
    }
  });

  it("matches the power-tools topic override regardless of language", () => {
    for (const language of languages) {
      const gifts = generateSmartFallbackGifts(quiz({ extraDetails: "serve un trapano" }), IT_COUNTRY, language);
      expect(gifts.some((g) => g.category === "power-tools")).toBe(true);
    }
  });
