...
describe("buildAmazonUrl", () => {
  ...
  it("falls back to a search link when no valid ASIN is available", () => {
    ...
    const url = buildAmazonUrl("Trapano Avvitatore Professionale Kit", IT_COUNTRY);
    expect(url).toBe("https://www.amazon.it/s?k=Trapano%20Avvitatore%20Professionale%20Kit&tag=test-tag-21");
  });
...
describe("buildAmazonCartUrl", () => {
  ...
  it("falls back to a search link (not a real cart add) without a real ASIN", () => {
    const url = buildAmazonCartUrl({ amazonSearchQuery: "Set Chiavi a Bussola 40 Pezzi" }, IT_COUNTRY);
    expect(url).toContain("/s?k=");
