...
export default function App() {
  ...
  const [showOnboardingHelp, setShowOnboardingHelp] = useState(() => {
    try {
      return !localStorage.getItem("bricolo_onboarding_seen");
    } catch (e) {
      return false;
    }
  });
  ...
  useEffect(() => {
    ...
    const metaDesc = language === "it"
      ? "Trova il prodotto giusto per casa, giardino e fai-da-te su Amazon con l'intelligenza artificiale. Veloce, preciso e senza stress."
      : "Find the right product for home, garden and DIY on Amazon with AI. Fast, accurate, and stress-free.";
    ...
    const title = language === "it"
      ? "Bricolo AI • L'Attrezzo Giusto in 3 Tap"
      : "Bricolo AI • The Right Tool in 3 Taps";
    ...
    if (typeof window !== "undefined") {
      ...
      if (action === "find_gift") {
        ...
        setQuizState((prev) => ({
          ...
          recipient,
          vibe,
          extraDetails: name ? `Progetto: ${name}` : prev.extraDetails,
        }));
        setShowSplash(false);
      }
    }
  ...
  const handleSendFeedback = useCallback(() => {
    if (typeof window !== "undefined") {
      window.open("mailto:support@bricolo.ai?subject=Feedback%20Bricolo%20AI", "_blank");
    }
  }, []);
  ...
  const handleDismissOnboardingHelp = useCallback(() => {
    ...
    try {
      localStorage.setItem("bricolo_onboarding_seen", "1");
    } catch (e) {}
  }, []);
